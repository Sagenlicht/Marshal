...
print "Registering Major Aura Motivate Urgency"
...
def auraBonus(attachee, args, evt_obj):
    ...
    auraBonus = args.get_arg(1)
    auraBonus *= 5
    evt_obj.bonus_list.add(auraBonus, 21,"~Circumstance~[TAG_MODIFIER_CIRCUMSTANCE] : ~{}~[TAG_AURA_{}]".format(auraName, auraTag))
    return 0

majorAura = PythonModifier("Major Aura Motivate Urgency", 3, False) #duration, auraBonus, auraEnum
majorAura.AddHook(ET_OnGetMoveSpeedBase, EK_NONE, auraBonus, ())
